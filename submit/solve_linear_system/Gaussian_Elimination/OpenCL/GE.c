...
"__kernel void eliminationKernel(__global float * d_matrix, __global int* pivot, int size)\n"\
"{\n"\
"    int i = get_global_id(0);\n"\
"    int j = get_global_id(1);\n"\
"    int lenY = get_global_size(1);\n"\
...
"__kernel void backSubstitution(__global float *d_matrix,  __global int* subLine, int size) {\n"\
"    int i = get_global_id(0);\n"\
"    int j = get_global_id(1);\n"\
"    int lenY = get_global_size(1);\n"\
"    if (i < (*subLine) && i >= 0) {\n"\
"        if (j == size) {\n"\
"            d_matrix[i * lenY + j] = d_matrix[i * lenY + j] - d_matrix[i * lenY + (*subLine)] * d_matrix[(*subLine) * lenY + size];\n"\
"        }\n"\
"    }\n"\
"}\n";

const char *ProgramSource4 =
"__kernel void setSubColToZero(__global float *d_matrix, __global int* subLine, int size) {\n"\
"    int i = get_global_id(0);\n"\
"    int j = get_global_id(1);\n"\
"    int lenY = get_global_size(1);\n"\
"    if (i < (*subLine) && i >= 0) {\n"\
...
int main(void) {
    ...
    cl_program program_eliminationKernel;
    cl_kernel kernel_eliminationKernel;
    cl_program program_setPivotColumnToZero;
    cl_kernel kernel_setPivotColumnToZero;
    cl_program program_backSubstitution;
    cl_kernel kernel_backSubstitution;
    cl_program program_setSubColToZero;
    cl_kernel kernel_setSubColToZero;
    ...
    // create program -- eliminationKernel
    program_eliminationKernel = clCreateProgramWithSource(context, 1, (const char**)&ProgramSource1, NULL, &err);
    // compile program
    if (clBuildProgram(program_eliminationKernel, 1, &device_id, NULL, NULL, NULL) != CL_SUCCESS) {
        printf("Error building program1\n");
        // Determine the size of the log
        size_t log_size;
        clGetProgramBuildInfo(program_eliminationKernel, device_id, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);

        // Allocate memory for the log
        char *log = (char *) malloc(log_size);
        // Get the log
        clGetProgramBuildInfo(program_eliminationKernel, device_id, CL_PROGRAM_BUILD_LOG, log_size, log, NULL);
    ...
    // create kernel
    kernel_eliminationKernel = clCreateKernel(program_eliminationKernel, "eliminationKernel", &err);
    ...
    // create program -- backSubstitution
    program_backSubstitution = clCreateProgramWithSource(context, 1, (const char**)&ProgramSource3, NULL, &err);
    // compile program
    if (clBuildProgram(program_backSubstitution, 1, &device_id, NULL, NULL, NULL) != CL_SUCCESS) {
        printf("Error building program3\n");
        // Determine the size of the log
        size_t log_size;
        clGetProgramBuildInfo(program_backSubstitution, device_id, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);

        // Allocate memory for the log
        char *log = (char *) malloc(log_size);
        // Get the log
        clGetProgramBuildInfo(program_backSubstitution, device_id, CL_PROGRAM_BUILD_LOG, log_size, log, NULL);
    ...
    // create kernel
    kernel_backSubstitution = clCreateKernel(program_backSubstitution, "backSubstitution", &err);


    // create program -- setSubColToZero
    program_setSubColToZero = clCreateProgramWithSource(context, 1, (const char**)&ProgramSource4, NULL, &err);
    // compile program
    if (clBuildProgram(program_setSubColToZero, 1, &device_id, NULL, NULL, NULL) != CL_SUCCESS) {
        printf("Error building program4\n");
        // Determine the size of the log
        size_t log_size;
        clGetProgramBuildInfo(program_setSubColToZero, device_id, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);

        // Allocate memory for the log
        char *log = (char *) malloc(log_size);
        // Get the log
        clGetProgramBuildInfo(program_setSubColToZero, device_id, CL_PROGRAM_BUILD_LOG, log_size, log, NULL);
    ...
    // create kernel
    kernel_setSubColToZero = clCreateKernel(program_setSubColToZero, "setSubColToZero", &err);
    ...
    clSetKernelArg(kernel_eliminationKernel, 0, sizeof(cl_mem), &d_matrix);
    clSetKernelArg(kernel_eliminationKernel, 1, sizeof(cl_mem), &d_pivot);
    clSetKernelArg(kernel_eliminationKernel, 2, sizeof(cl_int), &size);
    ...
    clSetKernelArg(kernel_backSubstitution, 0, sizeof(cl_mem), &d_matrix);
    clSetKernelArg(kernel_backSubstitution, 1, sizeof(cl_mem), &d_pivot);
    clSetKernelArg(kernel_backSubstitution, 2, sizeof(cl_int), &size);

    clSetKernelArg(kernel_setSubColToZero, 0, sizeof(cl_mem), &d_matrix);
    clSetKernelArg(kernel_setSubColToZero, 1, sizeof(cl_mem), &d_pivot);
    clSetKernelArg(kernel_setSubColToZero, 2, sizeof(cl_int), &size);
    ...
    for (i = 0; i < size; i++) {
        ...
        clEnqueueNDRangeKernel(command_queue, kernel_eliminationKernel, 2, 0, global, local, 0, NULL, NULL);
    ...
    for (i = size - 1; i > 0; i--) {
        int pivot = i;
        clEnqueueWriteBuffer(command_queue, d_pivot, CL_TRUE, 0, sizeof(int), &pivot, 0, NULL, NULL);
        clEnqueueNDRangeKernel(command_queue, kernel_backSubstitution, 2, 0, global, local, 0, NULL, NULL);
        clEnqueueNDRangeKernel(command_queue, kernel_setSubColToZero, 2, 0, global, local, 0, NULL, NULL);

        clEnqueueReadBuffer(command_queue, d_matrix, CL_TRUE, 0, sizeof(float)*data_size, matrix, 0, NULL, NULL);
    }
...
*/
    ...
    clReleaseProgram(program_eliminationKernel);
    clReleaseProgram(program_setPivotColumnToZero);
    clReleaseProgram(program_backSubstitution);
    clReleaseProgram(program_setSubColToZero);
    clReleaseProgram(program_writeToDResult);

    clReleaseKernel(kernel_changeFirstElementToOne);
    clReleaseKernel(kernel_eliminationKernel);
    clReleaseKernel(kernel_setPivotColumnToZero);
    clReleaseKernel(kernel_backSubstitution);
    clReleaseKernel(kernel_setSubColToZero);
